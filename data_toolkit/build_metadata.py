...
def update_metadata(path, opt):
    ...
    if opt.from_merged_records:
        df_files = [
            f
            for f in os.listdir(os.path.join(path, "merged_records"))
            if f.endswith(".csv")
        ]
        ...
        df_files = [
            f
            for f in os.listdir(os.path.join(path, "new_records"))
    ...
    if len(df_parts) > 0:
        ...
        for f in df_files:
            shutil.move(
                os.path.join(path, "new_records", f),
                os.path.join(path, "merged_records", f"{timestamp}_{f}"),
            )
        return metadata
    else:
        if os.path.exists(os.path.join(path, "metadata.csv")):
...
if __name__ == "__main__":
    dataset_utils = importlib.import_module(f"trellis_datasets.{sys.argv[1]}")
    ...
    # merge thumbnails
    thumbnail_metadata = update_metadata(
        os.path.join(opt.thumbnail_root, "thumbnails"), opt
    )

    # merge aesthetic scores
    aesthetic_score_metadata = update_metadata(
        os.path.join(opt.root, "aesthetic_scores"), opt
    )

    # merge render conditions
    render_cond_metadata = update_metadata(
        os.path.join(opt.render_cond_root, "renders_cond"), opt
    )

    # merge mesh dumped
    mesh_dumped_metadata = update_metadata(
        os.path.join(opt.mesh_dump_root, "mesh_dumps"), opt
    )

    # merge pbr dumped
    pbr_dumped_metadata = update_metadata(
        os.path.join(opt.pbr_dump_root, "pbr_dumps"), opt
    )
    ...
    for dir in os.listdir(opt.dual_grid_root):
        if os.path.isdir(os.path.join(opt.dual_grid_root, dir)) and dir.startswith(
            "dual_grid_"
        ):
            dual_grid_resolutions.append(int(dir.split("_")[-1]))
    dual_grid_metadata = {}
    for res in dual_grid_resolutions:
        dual_grid_metadata[res] = update_metadata(
            os.path.join(opt.dual_grid_root, f"dual_grid_{res}"), opt
        )

    # merge pbr voxelized
    pbr_voxel_resolutions = []
    for dir in os.listdir(opt.pbr_voxel_root):
        if os.path.isdir(os.path.join(opt.pbr_voxel_root, dir)) and dir.startswith(
            "pbr_voxels_"
        ):
            pbr_voxel_resolutions.append(int(dir.split("_")[-1]))
    pbr_voxel_metadata = {}
    for res in pbr_voxel_resolutions:
        pbr_voxel_metadata[res] = update_metadata(
            os.path.join(opt.pbr_voxel_root, f"pbr_voxels_{res}"), opt
        )
    ...
    for model in ss_latent_models:
        ss_latent_metadata[model] = update_metadata(
            os.path.join(opt.ss_latent_root, f"ss_latents/{model}"), opt
        )

    # merge shape latents
    shape_latent_models = []
    if os.path.exists(os.path.join(opt.shape_latent_root, "shape_latents")):
        shape_latent_models = os.listdir(
            os.path.join(opt.shape_latent_root, "shape_latents")
        )
    shape_latent_metadata = {}
    for model in shape_latent_models:
        shape_latent_metadata[model] = update_metadata(
            os.path.join(opt.shape_latent_root, f"shape_latents/{model}"), opt
        )
    ...
    for model in pbr_latent_models:
        pbr_latent_metadata[model] = update_metadata(
            os.path.join(opt.pbr_latent_root, f"pbr_latents/{model}"), opt
        )

    # statistics
    num_downloaded = (
        downloaded_metadata["local_path"].count()
        if downloaded_metadata is not None
        else 0
    )
    with open(os.path.join(opt.root, "statistics.txt"), "w") as f:
        ...
        if thumbnail_metadata is not None:
            f.write(
                f'  - Number of assets with thumbnails: {thumbnail_metadata["thumbnailed"].sum()}\n'
            )
        if aesthetic_score_metadata is not None:
            f.write(
                f'  - Number of assets with aesthetic scores: {aesthetic_score_metadata["aesthetic_score"].count()}\n'
            )
        if render_cond_metadata is not None:
            f.write(
                f'  - Number of assets with render conditions: {render_cond_metadata["cond_rendered"].count()}\n'
            )
        if mesh_dumped_metadata is not None:
            f.write(
                f'  - Number of assets with mesh dumped: {mesh_dumped_metadata["mesh_dumped"].sum()}\n'
            )
        if pbr_dumped_metadata is not None:
            f.write(
                f'  - Number of assets with PBR dumped: {pbr_dumped_metadata["pbr_dumped"].sum()}\n'
            )
        if asset_stats_metadata is not None:
            f.write(
                f"  - Number of assets with asset stats: {len(asset_stats_metadata)}\n"
            )
        if len(dual_grid_resolutions) != 0:
            ...
            for res in dual_grid_resolutions:
                if dual_grid_metadata[res] is not None:
                    f.write(
                        f'    - {res}: {dual_grid_metadata[res]["dual_grid_converted"].sum()}\n'
                    )
        if len(pbr_voxel_resolutions) != 0:
            ...
            for res in pbr_voxel_resolutions:
                if pbr_voxel_metadata[res] is not None:
                    f.write(
                        f'    - {res}: {pbr_voxel_metadata[res]["pbr_voxelized"].sum()}\n'
                    )
        if len(ss_latent_models) != 0:
            ...
            for model in ss_latent_models:
                if ss_latent_metadata[model] is not None:
                    f.write(
                        f'    - {model}: {ss_latent_metadata[model]["ss_latent_encoded"].sum()}\n'
                    )
        if len(shape_latent_models) != 0:
            ...
            for model in shape_latent_models:
                if shape_latent_metadata[model] is not None:
                    f.write(
                        f'    - {model}: {shape_latent_metadata[model]["shape_latent_encoded"].sum()}\n'
                    )
        if len(pbr_latent_models) != 0:
            ...
            for model in pbr_latent_models:
                if pbr_latent_metadata[model] is not None:
                    f.write(
                        f'    - {model}: {pbr_latent_metadata[model]["pbr_latent_encoded"].sum()}\n'
                    )
