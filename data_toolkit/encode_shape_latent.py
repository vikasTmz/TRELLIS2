...
if __name__ == "__main__":
    ...
    if opt.enc_model is None:
        ...
        print(type(encoder))  # shows full type incl. module path
        print(encoder.__class__.__name__)  # just the class name (e.g., "MyEncoder")
        print(encoder.__class__.__module__)  # module it came from
        print(f"{encoder.__class__.__module__}.{encoder.__class__.__name__}")

    else:
        latent_name = f'{opt.enc_model.split("/")[-1]}_{opt.ckpt}_{opt.resolution}'
        cfg = edict(
            json.load(
                open(os.path.join(opt.model_root, opt.enc_model, "config.json"), "r")
            )
        )
        encoder = getattr(models, cfg.models.encoder.name)(
            **cfg.models.encoder.args
        ).cuda()
        ckpt_path = os.path.join(
            opt.model_root, opt.enc_model, "ckpts", f"encoder_{opt.ckpt}.pt"
        )
    ...
    os.makedirs(
        os.path.join(
            opt.shape_latent_root, "shape_latents", latent_name, "new_records"
        ),
        exist_ok=True,
    )
    ...
    if os.path.exists(
        # os.path.join(opt.dual_grid_root, f"dual_grid_{opt.resolution}", "metadata.csv")
        os.path.join(
            opt.dual_grid_root, f"dual_grid_{opt.resolution}", "new_records/part_0.csv"
        )
    ):
        print("Found dual grid metadata")
        metadata = metadata.combine_first(
            pd.read_csv(
                os.path.join(
                    opt.dual_grid_root,
                    f"dual_grid_{opt.resolution}",
    ...
    with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor, tqdm(
        ...
        def check_sha256(sha256):
            if os.path.exists(
                os.path.join(
                    opt.shape_latent_root, "shape_latents", latent_name, f"{sha256}.npz"
                )
            ):
                coords = np.load(
                    os.path.join(
                ...
                records.append(
                    {
                        ...
                        "shape_latent_tokens": coords.shape[0],
                    }
                )
            pbar.update()
        ...
        for _ in tqdm(range(len(sha256s)), desc="Extracting latents"):
            try:
                ...
                pack = {
                    ...
                    "coords": z.coords[:, :].cpu().numpy().astype(np.uint8),
                }
