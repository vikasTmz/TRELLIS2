...
def _dual_grid_mesh(file, metadatum, mesh_dump_root, root):
    ...
    logging.info(f"Processing {sha256}...")
    try:
        pack = {"sha256": sha256}
        data = None
        for res in opt.resolution:
            ...
            if need_process:
                if data is None:
                    with open(
                        os.path.join(mesh_dump_root, "mesh_dumps", f"{sha256}.pickle"),
                        "rb",
                    ) as f:
                        dump = pickle.load(f)
                    start = 0
                    ...
                    assert torch.all(vertices >= -0.5) and torch.all(
                        vertices <= 0.5
                ...
                assert torch.all(dual_vertices >= -1e-3) and torch.all(
                    dual_vertices <= 1 + 1e-3
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mesh_dump_root", type=str, default=None, help="Directory to load mesh dumps"
    )
    parser.add_argument(
        "--dual_grid_root", type=str, default=None, help="Directory to save dual grids"
    )
    parser.add_argument(
    ...
    for res in opt.resolution:
        os.makedirs(
            os.path.join(opt.dual_grid_root, f"dual_grid_{res}", "new_records"),
            exist_ok=True,
        )
    ...
    if os.path.exists(
        os.path.join(opt.mesh_dump_root, "mesh_dumps/new_records", "part_0.csv")
    ):
        metadata = metadata.combine_first(
            pd.read_csv(
                os.path.join(opt.mesh_dump_root, "mesh_dumps/new_records", "part_0.csv")
    ...
    for res in opt.resolution:
        if os.path.exists(
            os.path.join(opt.dual_grid_root, f"dual_grid_{res}", "metadata.csv")
        ):
            dual_grid_metadata = pd.read_csv(
                os.path.join(opt.dual_grid_root, f"dual_grid_{res}", "metadata.csv")
            ).set_index("sha256")
            dual_grid_metadata = dual_grid_metadata.rename(
                columns={
                    "dual_grid_converted": f"dual_grid_converted_{res}",
                    "dual_grid_size": f"dual_grid_size_{res}",
                }
            )
    ...
    print(metadata)
    print(opt.dual_grid_root, opt.mesh_dump_root)

    # process objects
    func = partial(
        _dual_grid_mesh, root=opt.dual_grid_root, mesh_dump_root=opt.mesh_dump_root
    )
    ...
    for res in opt.resolution:
        if f"dual_grid_converted_{res}" in dual_grids.columns:
            dual_grid_metadata = dual_grids[
                dual_grids[f"dual_grid_converted_{res}"] == True
            ]
            if len(dual_grid_metadata) > 0:
                dual_grid_metadata = dual_grid_metadata[
                    ["sha256", f"dual_grid_converted_{res}", f"dual_grid_size_{res}"]
                ]
                dual_grid_metadata = dual_grid_metadata.rename(
                    columns={
                        f"dual_grid_converted_{res}": "dual_grid_converted",
                        f"dual_grid_size_{res}": "dual_grid_size",
                    }
                )
