...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory to load the shape latent files",
    )
    parser.add_argument(
        "--ss_latent_root",
        type=str,
    ...
    parser.add_argument(
        "--resolution", type=int, default=64, help="Sparse voxel resolution"
    )
    parser.add_argument(
        "--shape_latent_name",
        type=str,
    ...
    os.makedirs(
        os.path.join(opt.ss_latent_root, "ss_latents", latent_name, "new_records"),
        exist_ok=True,
    )
    ...
    if os.path.exists(
        os.path.join(opt.ss_latent_root, "ss_latents", latent_name, "metadata.csv")
    ):
        metadata = metadata.combine_first(
            pd.read_csv(
                os.path.join(
                    opt.ss_latent_root, "ss_latents", latent_name, "metadata.csv"
                )
            ).set_index("sha256")
        )
    ...
    with ThreadPoolExecutor(max_workers=32) as loader_executor, ThreadPoolExecutor(
        ...
        def loader(sha256):
            try:
                ...
                coords = coords[:, 1:]
                ss = torch.zeros(
                    1, opt.resolution, opt.resolution, opt.resolution, dtype=torch.long
                )
        ...
        def saver(sha256, pack):
            save_path = os.path.join(
                opt.ss_latent_root, "ss_latents", latent_name, f"{sha256}.npz"
            )
