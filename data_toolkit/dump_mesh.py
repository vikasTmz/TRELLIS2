...
def _install_blender():
    if not os.path.exists(BLENDER_PATH):
        ...
        os.system(
            "sudo apt-get install -y libxrender1 libxi6 libxkbcommon-x11-0 libsm6 libxfixes3 libgl1"
        )
        os.system(f"wget {BLENDER_LINK} -P {BLENDER_INSTALLATION_PATH}")
        os.system(
            f"tar -xvf {BLENDER_INSTALLATION_PATH}/blender-4.5.1-linux-x64.tar.xz -C {BLENDER_INSTALLATION_PATH}"
        )
...
if __name__ == "__main__":
    ...
    os.makedirs(
        os.path.join(opt.mesh_dump_root, "mesh_dumps", "new_records"), exist_ok=True
    )
    ...
    if os.path.exists(os.path.join(opt.download_root, "raw", "metadata.csv")):
        metadata = metadata.combine_first(
            pd.read_csv(
                os.path.join(opt.download_root, "raw", "metadata.csv")
            ).set_index("sha256")
        )
    if os.path.exists(os.path.join(opt.mesh_dump_root, "mesh_dumps", "metadata.csv")):
        metadata = metadata.combine_first(
            pd.read_csv(
                os.path.join(opt.mesh_dump_root, "mesh_dumps", "metadata.csv")
            ).set_index("sha256")
        )
    ...
    mesh_dumped.to_csv(
        os.path.join(
            opt.mesh_dump_root, "mesh_dumps", "new_records", f"part_{opt.rank}.csv"
        ),
        index=False,
    )
