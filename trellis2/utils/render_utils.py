...
def render_frames(sample, extrinsics, intrinsics, options={}, verbose=True, **kwargs):
    ...
    for j, (extr, intr) in tqdm(
        ...
        for k, v in res.items():
            ...
            rets[k].append(
                np.clip(
                    v.detach().cpu().numpy().transpose(1, 2, 0) * 255, 0, 255
                ).astype(np.uint8)
            )
    return rets
...
def render_video(
    sample, resolution=1024, bg_color=(0, 0, 0), num_frames=120, r=2, fov=40, **kwargs
):
...
def make_pbr_vis_frames(result, resolution=1024, input=None):
    ...
    for i in range(num_frames):
        ...
        base_color = base_color.resize((resolution, resolution))
        metallic = metallic.resize((resolution, resolution))
        roughness = roughness.resize((resolution // 2, resolution // 2))
        alpha = alpha.resize((resolution // 2, resolution // 2))
        if input is not None:
            # logging.info(input.size, shaded.size, normal.size)
            row1 = np.concatenate([input, normal, shaded], axis=1)
        else:
            row1 = base_color
        # row2 = np.concatenate([normal, base_color], axis=1)
