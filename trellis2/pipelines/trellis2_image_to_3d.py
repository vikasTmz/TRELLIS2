...
class Trellis2ImageTo3DPipeline(Pipeline):
    ...
    def sample_shape_slat_cascade(
        ...
        logging.info(
            f" \n\n Low-resolution noise slat shape: {noise.shape, noise.feats.shape, noise.coords.shape}  \n\n"
        )
        ...
        logging.info(f" \n\n Low-resolution shape slat shape: {slat.shape}  \n\n")
        ...
        logging.info(
            f" \n\n High-resolution noise slat shape: {noise.shape, noise.feats.shape, noise.coords.shape}  \n\n"
        )
        ...
        logging.info(f" \n\n High-resolution shape slat shape: {slat.shape}  \n\n")
    ...
    def decode_shape_slat(
        ...
        """
        print(
            f"Decoding shape SLat {slat.shape, slat.dtype, slat.device} to resolution {resolution}"
        )
    ...
    def run(
        ...
        print(f"\n\nUsing pipeline type: {pipeline_type}\n\n")
        if pipeline_type == "512":
            assert (
                "shape_slat_flow_model_512" in self.models
            ), "No 512 resolution shape SLat flow model found."
            assert (
                "tex_slat_flow_model_512" in self.models
            ), "No 512 resolution texture SLat flow model found."
        elif pipeline_type == "1024":
            assert (
                "shape_slat_flow_model_1024" in self.models
            ), "No 1024 resolution shape SLat flow model found."
            assert (
                "tex_slat_flow_model_1024" in self.models
