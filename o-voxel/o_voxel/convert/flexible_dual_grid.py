...
def _init_hashmap(grid_size, capacity, device):
    ...
    if VOL < 2**32:
        ...
        raise ValueError(
            f"The spatial size is too large to fit in a hashmap. Get volumn {VOL} > 2^64."
        )
...
@torch.no_grad()
def lines_to_flexible_dual_grid(
    ...
    """
    Voxelize a network of line segments into a sparse voxel grid.

    Args:
        vertices (torch.Tensor): The vertices of the lines (V, 3).
        lines (torch.Tensor): The line segments defined by vertex indices (L, 2).
        # ... [voxel_size, grid_size, aabb setup remains identical to the mesh version] ...
    """
    # Load geometry
    vertices = vertices.float()
    lines = lines.int()  # Shape (L, 2) instead of faces (F, 3)

    # Voxelize settings
    assert (
        voxel_size is not None or grid_size is not None
    ), "Either voxel_size or grid_size must be provided"
    ...
    if voxel_size is not None:
        ...
        assert (
            voxel_size.dim() == 1
        ), f"voxel_size must be a 1D tensor, but got {voxel_size.shape}"
        assert (
            voxel_size.size(0) == 3
        ), f"voxel_size must have 3 elements, but got {voxel_size.size(0)}"
    ...
    if grid_size is not None:
        ...
        assert (
            grid_size.dim() == 1
        ), f"grid_size must be a 1D tensor, but got {grid_size.shape}"
        assert (
            grid_size.size(0) == 3
    ...
    if aabb is None:
        ...
        if voxel_size is not None:
            padding = torch.ceil((max_xyz - min_xyz) / voxel_size) * voxel_size - (
                max_xyz - min_xyz
            )
            min_xyz -= padding * 0.5
            max_xyz += padding * 0.5
...
def flexible_dual_grid_to_mesh(
    ...
    edge_neighbor_voxel = (
        coords.reshape(N, 1, 1, 3)
        + flexible_dual_grid_to_mesh.edge_neighbor_voxel_offset
    )  # (N, 3, 4, 3)
    ...
    connected_voxel_indices = (
        _C.hashmap_lookup_3d_cuda(
            *hashmap, connected_voxel_hash_key, *grid_size.tolist()
        )
        .reshape(M, 4)
        .int()
    )
    ...
    # Construct triangles
    if not train:
        mesh_vertices = (coords.float() + dual_vertices) * voxel_size + aabb[0].reshape(
            1, 3
        )
        if split_weight is None:
            # if split 1
            atempt_triangles_0 = quad_indices[
                :, flexible_dual_grid_to_mesh.quad_split_1
            ]
            normals0 = torch.cross(
                mesh_vertices[atempt_triangles_0[:, 1]]
                - mesh_vertices[atempt_triangles_0[:, 0]],
                mesh_vertices[atempt_triangles_0[:, 2]]
                - mesh_vertices[atempt_triangles_0[:, 0]],
            )
            normals1 = torch.cross(
                mesh_vertices[atempt_triangles_0[:, 2]]
                - mesh_vertices[atempt_triangles_0[:, 1]],
                mesh_vertices[atempt_triangles_0[:, 3]]
                - mesh_vertices[atempt_triangles_0[:, 1]],
            )
            align0 = (normals0 * normals1).sum(dim=1, keepdim=True).abs()
            # if split 2
            atempt_triangles_1 = quad_indices[
                :, flexible_dual_grid_to_mesh.quad_split_2
            ]
            normals0 = torch.cross(
                mesh_vertices[atempt_triangles_1[:, 1]]
                - mesh_vertices[atempt_triangles_1[:, 0]],
                mesh_vertices[atempt_triangles_1[:, 2]]
                - mesh_vertices[atempt_triangles_1[:, 0]],
            )
            normals1 = torch.cross(
                mesh_vertices[atempt_triangles_1[:, 2]]
                - mesh_vertices[atempt_triangles_1[:, 1]],
                mesh_vertices[atempt_triangles_1[:, 3]]
                - mesh_vertices[atempt_triangles_1[:, 1]],
            )
            align1 = (normals0 * normals1).sum(dim=1, keepdim=True).abs()
            # select split
            mesh_triangles = torch.where(
                align0 > align1, atempt_triangles_0, atempt_triangles_1
            ).reshape(-1, 3)
        else:
        ...
        assert (
            split_weight is not None
        ), "split_weight must be provided in training mode"
        mesh_vertices = (coords.float() + dual_vertices) * voxel_size + aabb[0].reshape(
            1, 3
        )
        ...
        split_weight_ws_13 = split_weight_ws[:, 1] * split_weight_ws[:, 3]
        mid_vertices = (
            split_weight_ws_02 * mean_v02 + split_weight_ws_13 * mean_v13
        ) / (split_weight_ws_02 + split_weight_ws_13)
        mesh_vertices = torch.cat([mesh_vertices, mid_vertices], dim=0)
        quad_indices = torch.cat(
            [quad_indices, torch.arange(N, N + L, device="cuda").unsqueeze(1)], dim=1
        )
        mesh_triangles = quad_indices[
            :, flexible_dual_grid_to_mesh.quad_split_train
        ].reshape(-1, 3)
